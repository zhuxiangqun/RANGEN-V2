...
try:
    ...
    for exp, act in zip(expected, actual):
        if len(exp) > 20 and '.' in exp:
            long_text_total += 1
            if exp.lower() == act.lower() or exp.lower() in act.lower() or act.lower() in exp.lower():
                long_text_correct += 1
                print(f"  ✅ {exp[:50]} → {act[:50]}")
            else:
                print(f"  ❌ {exp[:50]} → {act[:50]}")
    
    if long_text_total > 0:
        long_text_rate = long_text_correct / long_text_total * 100
        print(f"  长文本答案准确率: {long_text_rate:.1f}% ({long_text_correct}/{long_text_total})")
    ...
    numerical_total = 0
    for exp, act in zip(expected, actual):
        if exp.isdigit() or (exp.endswith('th') and exp[:-2].isdigit()):
            numerical_total += 1
            if exp.lower() == act.lower() or exp.lower() in act.lower() or act.lower() in exp.lower():
                numerical_correct += 1
                print(f"  ✅ {exp} → {act}")
    ...
    if numerical_total > 0:
        numerical_rate = numerical_correct / numerical_total * 100
        print(f"  数字答案准确率: {numerical_rate:.1f}% ({numerical_correct}/{numerical_total})")
