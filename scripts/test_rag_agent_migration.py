...
async def run_rag_agent_migration_test():
    """运行RAGAgent迁移测试"""
    print("🚀 开始RAGAgent迁移测试")
    print("=" * 70)
    ...
    print("-" * 30)
    try:
        # 注意：RAGAgent实际上是RAGExpert的别名
        old_agent = RAGAgent()
        print("✅ 旧Agent创建成功 (RAGAgent)")
        new_agent = RAGExpert()
        print("✅ 新Agent创建成功 (RAGExpert)")
    ...
    for i, (rate, description) in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            print(f'   数据: {type(result.get("data"))}, keys = {list(result.get("data", {}).keys()) if isinstance(result.get("data"), dict) else "N/A"}')
...
if __name__ == '__main__':
    asyncio.run(run_rag_agent_migration_test())
