#!/usr/bin/env python3
"""
测试LearningSystem迁移状态
验证LearningSystemWrapper是否正确设置逐步替换策略
"""
...
async def test_learning_system_migration_status():
    """测试LearningSystem迁移状态"""
    print("🔍 测试LearningSystem迁移状态")
    print("=" * 60)
    ...
    try:
        # 测试逐步替换已启用的包装器
        wrapper = LearningSystemWrapper(enable_gradual_replacement=True, initial_replacement_rate=0.01)
        print("✅ LearningSystemWrapper实例化成功（逐步替换已启用）")
    ...
    try:
        ...
        print("✅ LearningSystemAdapter实例化成功")
    ...
    try:
        ...
        test_context = {
            'action': 'learn',
            'data': {'pattern': '用户查询优化', 'performance': 0.85}
        }
    ...
    try:
        ...
        # 检查是否使用了包装器
        if hasattr(system, 'learning_system') and system.learning_system is not None:
            agent_type = type(system.learning_system).__name__
            print(f"✅ 系统中的LearningSystem类型: {agent_type}")

            if agent_type == 'LearningSystemWrapper':
                print("✅ 系统正确使用LearningSystemWrapper")
                return True
            else:
                print(f"⚠️ 系统使用的是{agent_type}，不是LearningSystemWrapper")
                return False
        else:
            print("⚠️ 系统未初始化LearningSystem")
            return False
...
def main():
    """主函数"""
    success = asyncio.run(test_learning_system_migration_status())

    print(f'\n{"=" * 60}')
    if success:
        print("🎉 LearningSystem迁移状态验证通过！")
        print("✅ 所有组件正常工作")
        print("✅ 逐步替换策略已启用")
        print("✅ 系统集成正确")
        print("\n💡 迁移状态: 准备就绪，可以投入生产使用")
    else:
        print("❌ LearningSystem迁移状态验证失败！")
        print("需要检查和修复迁移问题")
