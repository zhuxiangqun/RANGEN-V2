#!/usr/bin/env python3
"""
DeepSeek专用生产环境功能测试脚本
验证DeepSeek配置和Agent实际执行性能（不使用OpenAI）
"""
...
try:
    ...
    # 在沙箱环境中跳过.env文件加载
    if os.name != 'nt':  # 非Windows环境（沙箱）
        load_dotenv()
    DOTENV_AVAILABLE = True
except (ImportError, PermissionError):
    DOTENV_AVAILABLE = False
    print("⚠️ dotenv模块不可用或权限受限，将直接使用环境变量")

async def test_api_keys():
    """测试API密钥配置（DeepSeek专用）"""
    print("🔑 测试API密钥配置（DeepSeek专用）...")
    print("-" * 40)
    ...
    # 检查OpenAI密钥（我们不使用，所以未配置是正常的）
    openai_key = os.getenv('OPENAI_API_KEY')
    results['OPENAI_API_KEY'] = {
        'configured': bool(openai_key),
        'required': False,  # DeepSeek专用模式下不需要
        'status': '正常' if not openai_key else '已配置（可选）'
    }

    # DeepSeek密钥检查
    ds_info = results['DEEPSEEK_API_KEY']
    ds_status = "✅ 已配置" if ds_info['configured'] else "❌ 未配置"
    ds_length_info = f"({ds_info['length']}字符)" if ds_info['configured'] else ""
    print(f"  DEEPSEEK_API_KEY: {ds_status} {ds_length_info}")
    if ds_info['configured'] and ds_info.get('masked'):
        print(f"    密钥前缀: {ds_info['masked']}")

    # OpenAI密钥检查（DeepSeek模式下不需要）
    oa_info = results['OPENAI_API_KEY']
    oa_status = "✅ 未配置（符合DeepSeek专用要求）" if not oa_info['configured'] else "ℹ️ 已配置（可选）"
    print(f"  OPENAI_API_KEY: {oa_status}")
...
async def generate_test_report(results):
    ...
    deepseek_configured = api_keys.get('DEEPSEEK_API_KEY', {}).get('configured', False)
    openai_configured = api_keys.get('OPENAI_API_KEY', {}).get('configured', False)

    print(f"🔑 DeepSeek密钥: {'✅ 已配置' if deepseek_configured else '❌ 未配置'}")
    print(f"🔑 OpenAI密钥: {'✅ 未配置（符合要求）' if not openai_configured else 'ℹ️ 已配置（可选）'}")
    ...
    # 总体评估（DeepSeek专用模式）
    # DeepSeek密钥 + Agent初始化 + 统一中心 + Agent执行 = 1 + 3 + 2 + 3 = 9分
    # OpenAI密钥不纳入评分（我们不使用）
    total_components = 9
    working_components = (1 if deepseek_configured else 0) + initialized_agents + working_centers + successful_executions

    success_rate = working_components / total_components * 100

    print(f"🎯 总体成功率: {success_rate:.1f}%")
    print(f"   • DeepSeek配置: {'✅' if deepseek_configured else '❌'}")
    print(f"   • Agent初始化: ✅ ({initialized_agents}/3)")
    print(f"   • 统一中心: ✅ ({working_centers}/2)")
    print(f"   • Agent执行: ✅ ({successful_executions}/3)")

    if success_rate >= 80:
        print("🎉 DeepSeek专用生产环境测试通过！系统运行正常")
        print("   💡 系统已正确配置为仅使用DeepSeek")
    elif success_rate >= 60:
        print("⚠️ 生产环境测试基本通过，但存在一些问题需要关注")
        if not deepseek_configured:
            print("   💡 建议：配置DEEPSEEK_API_KEY环境变量")
    else:
