#!/usr/bin/env python3
"""
简化版Agent清理脚本 - 100%迁移后清理不再需要的旧文件
"""
...
def analyze_current_state():
    ...
    print(f"📊 总共发现 {len(all_py_files)} 个Python文件")
    ...
    for agent in core_agents:
        status = "✅" if any(f.name == agent for f in all_py_files) else "❌"
...
def perform_cleanup(cleanup_files):
    """执行清理"""
    print("\n🗑️  执行清理")
    print("-" * 40)
    ...
    confirm = input("\n确认删除这些文件？(输入 'yes' 确认): ").strip().lower()
    if confirm != 'yes':
        print("❌ 清理已取消")
        return False
...
def main():
    ...
    # 分析当前状态
    cleanup_files = analyze_current_state()
    ...
    # 执行清理
    if perform_cleanup(cleanup_files):
        # 更新__init__.py
        update_init_file()

        print("\n🎉 清理完成！")
        print("✅ 系统现在只包含8个核心Agent")
        print("✅ 所有旧代码已清理并备份")
        ...
        # 验证8个核心Agent都在
        core_agents = [
            'rag_agent.py', 'reasoning_expert.py', 'agent_coordinator.py',
            'quality_controller.py', 'tool_orchestrator.py', 'memory_manager.py',
            'learning_optimizer.py', 'chief_agent.py'
        ]
