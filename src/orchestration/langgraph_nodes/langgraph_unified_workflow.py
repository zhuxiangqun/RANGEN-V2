...
try:
    from src.orchestration.langgraph_opentelemetry_integration import (
        traced_node,
...
try:
    from src.orchestration.langgraph_nodes.evidence_check_node import EvidenceCheckNode
...
class UnifiedResearchWorkflow:
    ...
    def __init__(self, system=None, use_persistent_checkpoint: Optional[bool] = None):
        ...
        # 🚀 优化：状态版本管理器
        try:
            from src.orchestration.langgraph_state_version_manager import StateVersionManager
        ...
        # 🚀 优化：动态工作流管理器
        try:
            from src.orchestration.langgraph_dynamic_workflow import DynamicWorkflowManager
        ...
        # 🚀 优化：错误恢复器
        try:
            from src.orchestration.langgraph_error_recovery import CheckpointErrorRecovery
        ...
        # 🚀 优化：性能优化器（缓存、LLM优化器）
        try:
            from src.orchestration.langgraph_performance_optimizer import (
                get_workflow_cache,
                get_llm_optimizer,
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        try:
            from src.orchestration.langgraph_nodes.langgraph_reasoning_nodes import ReasoningNodes
        ...
        try:
            print("🔧 初始化AgentNodes...")
            from src.orchestration.langgraph_nodes.langgraph_agent_nodes import AgentNodes
        ...
        core_nodes = None
        try:
            from src.orchestration.langgraph_nodes.langgraph_core_nodes import CoreNodes
        ...
        detailed_nodes = None
        try:
            from src.orchestration.langgraph_nodes.langgraph_detailed_processing_nodes import DetailedProcessingNodes
        ...
        if core_nodes:
            ...
            if use_performance_monitoring:
                from src.orchestration.langgraph_performance_monitor import monitor_performance
        ...
        from src.orchestration.langgraph_nodes.langgraph_config_nodes import (
        ...
        # 🚀 阶段6：添加学习节点
        from src.orchestration.langgraph_nodes.langgraph_learning_nodes import (
        ...
        # 🚀 阶段7：添加能力架构节点
        from src.orchestration.langgraph_nodes.langgraph_capability_nodes import (
        ...
        if use_subgraph_encapsulation:
            try:
                from src.orchestration.langgraph_subgraph_builder import ReasoningSubgraphBuilder, MultiAgentSubgraphBuilder
        ...
        if self.agent_nodes and memory_agent_func:
            ...
            if knowledge_retrieval_agent_func:
                if enable_fallback_routing:
                    # 启用备用路由：创建备用节点并配置条件边
                    try:
                        from src.orchestration.langgraph_enhanced_error_recovery import EnhancedErrorRecovery
            ...
            if enable_parallel_execution and knowledge_retrieval_agent_func and reasoning_agent_func:
                ...
                try:
                    from src.orchestration.langgraph_parallel_execution import parallel_merge_node
                    # 添加并行合并节点
        ...
        try:
            ...
            if os.getenv('ENABLE_PERFORMANCE_MONITOR_NODE', 'false').lower() == 'true':
                from src.orchestration.langgraph_performance_monitor import performance_monitor_node
        ...
        try:
            ...
            if retrieval_result and retrieval_result.success:
                ...
                if sources:
                    ...
                    import os
                    from src.orchestration.llm_integration import LLMIntegration
...
答案:"""
    ...
    def _route_decision(self, state: ResearchSystemState) -> str:
        ...
        try:
            ...
            if os.getenv('ENABLE_CONFIGURABLE_ROUTER', 'false').lower() == 'true':
                from src.orchestration.langgraph_configurable_router import get_configurable_router
    ...
    def _add_dynamic_capability_nodes(self, workflow) -> List[str]:
        ...
        try:
            # 导入能力节点工厂函数
            from src.orchestration.langgraph_nodes.langgraph_capability_nodes import create_capability_node
