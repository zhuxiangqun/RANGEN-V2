#!/usr/bin/env python3
"""
核心系统强化学习优化器 - 阶段3：强化学习
使用Q-learning优化策略选择（模型选择、证据选择等）
"""
...
class ReinforcementLearningOptimizer:
    """核心系统强化学习优化器（Q-learning）"""
    
    def __init__(
        self,
        data_path: str = "data/learning/rl_optimization.json",
    ...
    def _state_to_key(self, state: State) -> str:
        ...
        complexity_bin = 'low' if state.complexity_score < 0.3 else 'medium' if state.complexity_score < 0.7 else 'high'
        evidence_bin = 'few' if state.evidence_count < 3 else 'medium' if state.evidence_count < 10 else 'many'
        
        return f"{query_type}_{query_length_bin}_{complexity_bin}_{evidence_bin}"
    
    def _action_to_key(self, action: Action) -> str:
        """将动作转换为Q表的键"""
        return f"{action.model_type}_e{action.evidence_count}_c{action.complexity_threshold:.2f}_conf{action.confidence_threshold:.2f}"
    ...
    def select_action(self, state: State, available_actions: List[Action]) -> Action:
        ...
        if np.random.random() < self.epsilon:
            ...
            self.logger.debug(f"探索：随机选择动作 {action.model_type}")
        else:
            # 利用：选择Q值最高的动作
            best_action = None
            ...
            self.logger.debug(f"利用：选择Q值最高的动作 {action.model_type} (Q={best_q_value:.4f})")
    ...
    def calculate_reward(
        ...
        efficiency_reward = 1.0 / (1.0 + processing_time / 100.0)  # 归一化（假设100秒为基准）
        
        # 证据数量奖励（适中的证据数量更好）
        if 3 <= evidence_count <= 10:
            count_reward = 1.0
