...
@dataclass
class DynamicSkill:
    """动态 Skill 包装器 - 简单的 Skill 实例"""
    metadata: Any  # SkillMetadata
    config: Dict[str, Any]
    name: str = ""
    
    def __post_init__(self):
        if not self.name and hasattr(self.metadata, 'name'):
            self.name = self.metadata.name
    
    @property
    def triggers(self) -> List[str]:
        return getattr(self.metadata, 'triggers', [])
    
    @property
    def prompt_template(self) -> str:
        return getattr(self.metadata, 'prompt_template', "")
