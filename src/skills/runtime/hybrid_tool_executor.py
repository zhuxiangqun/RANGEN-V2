...
class HybridToolExecutor:
    ...
    def __init__(self, internal_mode: str = "direct", enable_tool_mapping: bool = True):
        """
        初始化混合工具执行器
        ...
        Args:
            ...
            enable_tool_mapping: 是否启用工具到Skill的映射
        """
        ...
        self.enable_tool_mapping = enable_tool_mapping
        self._tool_mapper = None
        if enable_tool_mapping:
            self._init_tool_mapper()
    
    def _init_tool_mapper(self):
        """初始化工具到Skill的映射"""
        try:
            from src.skills.runtime.tool_to_skill_map import get_skill_name
            self._tool_mapper = get_skill_name
            logger.info("Tool to Skill mapping enabled")
        except ImportError:
            logger.warning("ToolToSkillMap not available, mapping disabled")
            self.enable_tool_mapping = False
    ...
    async def execute(
        ...
        # 0. 工具映射 (Tool → Skill)
        if self.enable_tool_mapping and self._tool_mapper:
            mapped_tool = self._tool_mapper(tool_name)
