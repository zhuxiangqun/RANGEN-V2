...
if TYPE_CHECKING:
    from src.orchestration.self_evolving_agent import EvolutionConfig, EvolutionStrategy, AgentState
    from src.orchestration.self_evolving_agent import SelfEvolvingAgent, EvolutionResult

from src.orchestration.self_evolving_agent import (
    SelfEvolvingAgent, EvolutionConfig, EvolutionStrategy,
    EvolutionStage, AgentState, EvolutionResult
)
from src.orchestration.reflection import ReflexionAgent, ReflectionResult
from src.orchestration.multi_trajectory import MultiTrajectoryExplorer, ExplorerConfig
...
class SelfEvolvingMixin:
    ...
    def _init_kms_connection(self) -> None:
        try:
            ...
            self._kms_client = get_kms_client()
            logger.info(f"{agent_id}: Connected to KMS")
