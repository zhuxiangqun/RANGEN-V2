#!/usr/bin/env python3
"""
ChiefAgent包装器 - 使用逐步替换策略

将ChiefAgent的调用包装为使用逐步替换策略，实现平滑迁移到AgentCoordinator。
"""
...
from ..adapters.chief_agent_adapter import ChiefAgentAdapter
from ..strategies.gradual_replacement import GradualReplacementStrategy

logger = logging.getLogger(__name__)


class ChiefAgentWrapper(BaseAgent):
    """ChiefAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与ChiefAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从ChiefAgent迁移到AgentCoordinator。
    """
    ...
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 1.0):
        ...
        config = AgentConfig(
            ...
            agent_type="chief_agent_wrapper"
        )
        super().__init__(
            "chief_agent",
            ["coordination", "task_decomposition", "team_management", 
             "conflict_resolution", "cognitive_partnership"],
            config
        )
        
        # 创建旧Agent实例
        self.old_agent = ChiefAgent()
        ...
        if enable_gradual_replacement:
            ...
            logger.info("ℹ️ ChiefAgent包装器初始化成功，逐步替换未启用")
    ...
    @property
    def expert_agent_pool(self):
        """专家Agent池（代理到old_agent）"""
        return self.old_agent.expert_agent_pool
