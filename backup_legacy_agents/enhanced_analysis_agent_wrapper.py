#!/usr/bin/env python3
"""
EnhancedAnalysisAgent包装器 - 使用逐步替换策略

将EnhancedAnalysisAgent的调用包装为使用逐步替换策略，实现平滑迁移到ReasoningExpert。
"""
...
from ..adapters.enhanced_analysis_agent_adapter import EnhancedAnalysisAgentAdapter
from ..strategies.gradual_replacement import GradualReplacementStrategy

logger = logging.getLogger(__name__)


class EnhancedAnalysisAgentWrapper(BaseAgent):
    """EnhancedAnalysisAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与EnhancedAnalysisAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从EnhancedAnalysisAgent迁移到ReasoningExpert。
    """
    
    def __init__(self, config: Optional[AgentConfig] = None, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 0.01):
        """
        初始化EnhancedAnalysisAgent包装器
        ...
        if config is None:
            config = AgentConfig(
                ...
                agent_type="enhanced_analysis_agent_wrapper"
            )
        super().__init__("enhanced_analysis_agent", ["analysis", "reasoning"], config)
        
        # 创建旧Agent实例
        self.old_agent = EnhancedAnalysisAgent(config)
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ EnhancedAnalysisAgent包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ EnhancedAnalysisAgent包装器初始化成功，逐步替换未启用")
