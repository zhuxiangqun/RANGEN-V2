#!/usr/bin/env python3
"""
ContextEngineeringAgent包装器 - 使用逐步替换策略

将ContextEngineeringAgent的调用包装为使用逐步替换策略，实现平滑迁移到MemoryManager。
"""
...
from ..adapters.context_engineering_agent_adapter import ContextEngineeringAgentAdapter
from ..strategies.gradual_replacement import GradualReplacementStrategy

logger = logging.getLogger(__name__)


class ContextEngineeringAgentWrapper(BaseAgent):
    """ContextEngineeringAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与ContextEngineeringAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从ContextEngineeringAgent迁移到MemoryManager。
    """
    
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 1.0):
        """
        初始化ContextEngineeringAgent包装器
        ...
        config = AgentConfig(
            ...
            agent_type="context_engineering_agent_wrapper"
        )
        super().__init__("context_engineering_expert", ["context_engineering", "memory_management"], config)
        
        # 创建旧Agent实例
        self.old_agent = ContextEngineeringAgent()
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ ContextEngineeringAgent包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ ContextEngineeringAgent包装器初始化成功，逐步替换未启用")
