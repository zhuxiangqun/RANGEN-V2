#!/usr/bin/env python3
"""
MemoryAgent包装器 - 使用逐步替换策略

将MemoryAgent的调用包装为使用逐步替换策略，实现平滑迁移到MemoryManager。
"""
...
class MemoryAgentWrapper(BaseAgent):
    ...
    这个包装器实现了与MemoryAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从MemoryAgent迁移到MemoryManager。
    """
    ...
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 0.01):
        ...
        config = AgentConfig(
            ...
            agent_type="memory_agent_wrapper"
        )
        super().__init__("memory_expert", ["memory", "context_management"], config)
        
        # 创建旧Agent实例
        self.old_agent = MemoryAgent()
        ...
        if enable_gradual_replacement:
            ...
            logger.info("ℹ️ MemoryAgent包装器初始化成功，逐步替换未启用")
