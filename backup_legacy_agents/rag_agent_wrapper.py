#!/usr/bin/env python3
"""
RAGAgent包装器 - 使用逐步替换策略

注意：RAGAgent实际上是RAGExpert的别名（RAGAgent = RAGExpert），
所以这个包装器主要是为了保持迁移框架的一致性。
实际上可以直接使用RAGExpert，但通过包装器可以统一管理迁移过程和日志。
"""

import logging
from typing import Dict, Any, Optional
from .rag_agent import RAGExpert  # RAGAgent是RAGExpert的别名
from .base_agent import BaseAgent
from ..adapters.rag_agent_adapter import RAGAgentAdapter
...
class RAGAgentWrapper(BaseAgent):
    ...
    注意：由于RAGAgent是RAGExpert的别名，这个包装器主要是为了保持迁移框架的一致性。
    实际上可以直接使用RAGExpert，但通过包装器可以统一管理迁移过程和日志。
    """
    ...
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 0.01):
        ...
        config = AgentConfig(
            agent_id="rag_expert",
            agent_type="rag_agent_wrapper"
        )
        super().__init__("rag_expert", ["rag", "knowledge_retrieval", "answer_generation"], config)
        
        # 创建旧Agent实例（RAGAgent是RAGExpert的别名）
        self.old_agent = RAGExpert()
        ...
        if enable_gradual_replacement:
            ...
            logger.info("ℹ️ RAGAgent包装器初始化成功，逐步替换未启用")
