#!/usr/bin/env python3
"""
CitationAgent包装器 - 使用逐步替换策略

将CitationAgent的调用包装为使用逐步替换策略，实现平滑迁移到QualityController。
"""

import logging
from typing import Dict, Any, Optional
from backup_legacy_agents.expert_agents import CitationAgent
...
class CitationAgentWrapper(BaseAgent):
    """CitationAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与CitationAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从CitationAgent迁移到QualityController。
    """
    ...
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 0.01):
        ...
        config = AgentConfig(
            ...
            agent_type="citation_agent_wrapper"
        )
        super().__init__("citation_expert", ["citation", "quality_control"], config)
        
        # 创建旧Agent实例
        self.old_agent = CitationAgent()
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ CitationAgent包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ CitationAgent包装器初始化成功，逐步替换未启用")
