#!/usr/bin/env python3
"""
FactVerificationAgent包装器 - 使用逐步替换策略

将FactVerificationAgent的调用包装为使用逐步替换策略，实现平滑迁移到QualityController。
注意：FactVerificationAgent不继承BaseAgent，是一个普通类。
"""
...
from ..adapters.fact_verification_agent_adapter import FactVerificationAgentAdapter
from ..strategies.gradual_replacement import GradualReplacementStrategy

logger = logging.getLogger(__name__)


class FactVerificationAgentWrapper(BaseAgent):
    """FactVerificationAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与FactVerificationAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从FactVerificationAgent迁移到QualityController。
    """
    
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 0.01):
        """
        初始化FactVerificationAgent包装器
        ...
        config = AgentConfig(
            ...
            agent_type="fact_verification_agent_wrapper"
        )
        super().__init__("fact_verification_agent", ["fact_verification", "quality_control"], config)
        
        # 创建旧Agent实例（FactVerificationAgent不继承BaseAgent）
        self.old_agent = FactVerificationAgent()
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ FactVerificationAgent包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ FactVerificationAgent包装器初始化成功，逐步替换未启用")
