#!/usr/bin/env python3
"""
LearningSystem包装器 - 使用逐步替换策略

将LearningSystem的调用包装为使用逐步替换策略，实现平滑迁移到LearningOptimizer。
"""
...
from ..adapters.learning_system_adapter import LearningSystemAdapter
from ..strategies.gradual_replacement import GradualReplacementStrategy

logger = logging.getLogger(__name__)


class LearningSystemWrapper(BaseAgent):
    """LearningSystem包装器 - 使用逐步替换策略
    
    这个包装器实现了与LearningSystem相同的接口，但内部使用逐步替换策略
    将请求逐步从LearningSystem迁移到LearningOptimizer。
    """
    
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 1.0):
        """
        初始化LearningSystem包装器
        ...
        config = AgentConfig(
            ...
            agent_type="learning_system_wrapper"
        )
        super().__init__("learning_system", ["learning", "optimization"], config)
        
        # 创建旧Agent实例
        self.old_agent = LearningSystem()
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ LearningSystem包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ LearningSystem包装器初始化成功，逐步替换未启用")
    ...
    async def execute(self, context: Dict[str, Any]) -> Any:
        ...
        if self.enable_gradual_replacement and self.replacement_strategy:
            ...
            try:
                ...
                # 回退到旧Agent
                if hasattr(self.old_agent, 'execute'):
                    return await self.old_agent.execute(context)
                else:
                    # LearningSystem可能没有execute方法，需要特殊处理
