#!/usr/bin/env python3
"""
PromptEngineeringAgent包装器 - 使用逐步替换策略

将PromptEngineeringAgent的调用包装为使用逐步替换策略，实现平滑迁移到ToolOrchestrator。
"""
...
from ..adapters.prompt_engineering_agent_adapter import PromptEngineeringAgentAdapter
from ..strategies.gradual_replacement import GradualReplacementStrategy

logger = logging.getLogger(__name__)


class PromptEngineeringAgentWrapper(BaseAgent):
    """PromptEngineeringAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与PromptEngineeringAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从PromptEngineeringAgent迁移到ToolOrchestrator。
    """
    
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 1.0):
        """
        初始化PromptEngineeringAgent包装器
        ...
        config = AgentConfig(
            ...
            agent_type="prompt_engineering_agent_wrapper"
        )
        super().__init__("prompt_engineering_expert", ["prompt_engineering", "tool_orchestration"], config)
        
        # 创建旧Agent实例
        self.old_agent = PromptEngineeringAgent()
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ PromptEngineeringAgent包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ PromptEngineeringAgent包装器初始化成功，逐步替换未启用")
    ...
    def prompt_engine(self):
        """提示词引擎（代理到old_agent）"""
        return self.old_agent.prompt_engine
