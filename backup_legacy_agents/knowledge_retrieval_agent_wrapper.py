#!/usr/bin/env python3
"""
KnowledgeRetrievalAgent包装器 - 使用逐步替换策略

将KnowledgeRetrievalAgent的调用包装为使用逐步替换策略，实现平滑迁移到RAGExpert。
"""

import logging
from typing import Dict, Any, Optional
from backup_legacy_agents.expert_agents import KnowledgeRetrievalAgent
...
class KnowledgeRetrievalAgentWrapper(BaseAgent):
    """KnowledgeRetrievalAgent包装器 - 使用逐步替换策略
    
    这个包装器实现了与KnowledgeRetrievalAgent相同的接口，但内部使用逐步替换策略
    将请求逐步从KnowledgeRetrievalAgent迁移到RAGExpert。
    """
    
    def __init__(self, enable_gradual_replacement: bool = True, initial_replacement_rate: float = 0.01):
        """
        初始化KnowledgeRetrievalAgent包装器
        ...
        config = AgentConfig(
            ...
            agent_type="knowledge_retrieval_agent_wrapper"
        )
        super().__init__("knowledge_retrieval_expert", ["knowledge_retrieval", "rag"], config)
        
        # 创建旧Agent实例
        self.old_agent = KnowledgeRetrievalAgent()
        ...
        if enable_gradual_replacement:
            try:
                ...
                logger.info(
                    f"✅ KnowledgeRetrievalAgent包装器初始化成功，逐步替换已启用 "
            ...
            logger.info("ℹ️ KnowledgeRetrievalAgent包装器初始化成功，逐步替换未启用")
