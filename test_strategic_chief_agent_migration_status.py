#!/usr/bin/env python3
"""
测试StrategicChiefAgent迁移状态
验证StrategicChiefAgentWrapper是否正确设置逐步替换策略
"""
...
async def test_strategic_chief_agent_migration_status():
    """测试StrategicChiefAgent迁移状态"""
    print("🔍 测试StrategicChiefAgent迁移状态")
    print("=" * 60)
    ...
    try:
        # 测试逐步替换已启用的包装器
        wrapper = StrategicChiefAgentWrapper(enable_gradual_replacement=True, initial_replacement_rate=0.01)
        print("✅ StrategicChiefAgentWrapper实例化成功（逐步替换已启用）")
    ...
    try:
        ...
        print("✅ StrategicChiefAgentAdapter实例化成功")
        ...
        for file_path in files_to_check:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if 'StrategicChiefAgentWrapper' in content:
...
def main():
    """主函数"""
    success = asyncio.run(test_strategic_chief_agent_migration_status())

    print(f'\n{"=" * 60}')
    if success:
        print("🎉 StrategicChiefAgent迁移状态验证通过！")
        print("✅ 所有组件正常工作")
        print("✅ 逐步替换策略已启用")
        print("✅ 系统集成正确")
        print("\n💡 迁移状态: 准备就绪，可以投入生产使用")
    else:
        print("❌ StrategicChiefAgent迁移状态验证失败！")
        print("需要检查和修复迁移问题")
