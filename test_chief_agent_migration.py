...
"""
ChiefAgent迁移测试脚本
测试ChiefAgent → AgentCoordinator的迁移功能
"""
...
async def run_chief_agent_migration_test():
    """运行ChiefAgent迁移测试"""
    print("🚀 开始ChiefAgent迁移测试")
    print("=" * 70)
    ...
    print("-" * 30)
    try:
        old_agent = ChiefAgent()
        print("✅ 旧Agent创建成功 (ChiefAgent)")
        new_agent = AgentCoordinator()
        print("✅ 新Agent创建成功 (AgentCoordinator)")
    ...
    test_scenarios = [
        (0.0, "全部使用旧Agent (ChiefAgent)"),
        (0.5, "50%概率使用新Agent (AgentCoordinator)，50%使用旧Agent"),
        (1.0, "全部使用新Agent (AgentCoordinator)")
    ]
    ...
    for i, (rate, description) in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            if isinstance(data, dict):
                print(f'   数据: dict, keys = {list(data.keys())[:5]}...')  # 只显示前5个key
            else:
    ...
    if len(successful_tests) == len(test_scenarios):
        print("\n🎉 恭喜！迁移测试完全成功！")
        print("✅ ChiefAgent成功迁移到AgentCoordinator")
    ...
    print("=" * 40)
    if len(successful_tests) == len(test_scenarios):
        print("✅ ChiefAgent → AgentCoordinator 迁移成功！")
...
if __name__ == '__main__':
    asyncio.run(run_chief_agent_migration_test())
