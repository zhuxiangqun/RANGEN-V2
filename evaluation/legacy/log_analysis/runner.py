...
class LogBasedEvaluator:
    ...
    def __init__(self, log_file: Optional[str] = None, sample_count: int = 50, max_concurrent: int = 4):
        # 先初始化logger
        ...
        self.static_architecture_analyzer = StaticArchitectureAnalyzer()
        self.static_data_analyzer = StaticDataCapabilityAnalyzer()
        self.static_platform_analyzer = StaticPlatformCapabilityAnalyzer()
    ...
    def analyze_log_file(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"使用样本数限制: {self.sample_count}")
            
            sample_count = self._extract_sample_count(log_content)
            
            # 根据样本数限制日志范围（按样本ID分段）
            if self.sample_count > 0 and self.sample_count < sample_count:
                limited_content = self._limit_log_by_sample_count(log_content, self.sample_count)
                if limited_content:
                    log_content = limited_content
                    self.logger.info(f"已限制分析范围为前 {self.sample_count} 个样本")
            ...
            analysis_result = {
                ...
                # 静态评估
                "code_quality_analysis": self.code_quality_analyzer.analyze(),
                "static_architecture_analysis": self.static_architecture_analyzer.analyze(),
                "static_data_analysis": self.static_data_analyzer.analyze(),
                "static_platform_analysis": self.static_platform_analyzer.analyze(),
    ...
    def _limit_log_by_sample_count(self, log_content: str, sample_count: int) -> Optional[str]:
        import re
        ...
        for pattern in sample_patterns:
            ...
            for match in matches:
                sample_positions.append((match.start(), match.group(1) if match.groups() else match.group(0)))
        
        if not sample_positions:
            return log_content
        
        sample_positions.sort(key=lambda x: x[0])
        
        seen_samples = set()
        cutoff_pos = len(log_content)
        
        for pos, sample_id in sample_positions:
            if sample_id not in seen_samples:
                seen_samples.add(sample_id)
                if len(seen_samples) >= sample_count:
                    break
            else:
                cutoff_pos = pos
        
        if len(seen_samples) >= sample_count:
            return log_content[:cutoff_pos]
        return log_content
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="RANGEN 基于日志的性能评测系统")
    parser.add_argument("--sample-count", type=int, default=50, help="最大样本数量 (默认: 50)")
    parser.add_argument("--max-concurrent", type=int, default=4, help="最大并发分析数 (默认: 4)")
    ...
    print(f"📊 基于日志的性能评测 - 样本数: {args.sample_count}, 并发数: {args.max_concurrent}")
    
    # 创建评测器
    evaluator = LogBasedEvaluator()
    evaluator.sample_count = args.sample_count
    evaluator.max_concurrent = args.max_concurrent
    ...
    # 保存MD报告文件到log_analysis/results目录
    import os
    from datetime import datetime
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    report_dir = "evaluation/log_analysis/results"
    os.makedirs(report_dir, exist_ok=True)
    report_filename = os.path.join(report_dir, f"log_analysis_report_{timestamp}.md")
    result_filename = os.path.join(report_dir, f"log_analysis_{timestamp}.json")
    ...
    # 保存结果
    evaluator.save_results(result_filename)
    print("评测完成！")
