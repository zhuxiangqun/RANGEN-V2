...
class UnifiedTokenizationManager:
    ...
    def _load_tokenizers(self):
        ...
        try:
            ...
            self.logger.warning("多语言分词器加载失败: {e}")
        ...
        try:
            ...
            self.logger.warning("传统分词器加载失败: {e}")
    ...
    def select_strategy(self, text: str, task_type: str = "general",
        ...
        if task_type in ["classification", "sentiment", "ner", "pos_tagging"]:
            if chinese_ratio > get_smart_config("medium_threshold", {"config_type": "auto"}, create_query_context(query_type="medium_threshold")) and TokenizationStrategy.TRADITIONAL in self.tokenizers:
                self.logger.info("选择传统分词策略(中文NLP任务)")
                return TokenizationStrategy.TRADITIONAL

        if chinese_ratio < get_smart_config("high_threshold", {"config_type": "auto"}, create_query_context(query_type="high_threshold")) and chinese_ratio > 0.2:  # 混合语言
        ...
        return [
            {
                "word": word,
                "type": "fallback",
                "confidence": get_smart_config("low_threshold", {"config_type": "auto"}, create_query_context(query_type="low_threshold")),
                "strategy": "fallback"
            }
